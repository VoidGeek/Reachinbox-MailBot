...
export const categorizeEmail = async (subject: string,content: string): Promise<string> => {
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {role: "system", content: "You are an AI assistant that categorizes emails. Categorize the email as either 'Interested', 'Not Interested', or 'More Information'. Respond with only the category."},
